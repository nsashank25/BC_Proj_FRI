...
// --- End of REVISED Buy Tokens Function ---
  ...
  const handleApproveMarketplace = async () => {
    ...
    // Check if user has any tokens to approve (removed owner requirement)
    if (parseFloat(propertyDetails.myBalance) <= 0) {
        alert("You don't have any tokens to approve for sale.");
        return;
    }
    ...
    try {
        ...
        // Approve only the tokens the user has (instead of total supply)
        const myTokens = await tokenContract.balanceOf(account);
        console.log(`Approving marketplace for user's tokens: ${myTokens.toString()}`);

        // Send the approve transaction with user's token amount
        const approveTx = await tokenContract.approve(
            TOKEN_MARKETPLACE_ADDRESS,
            myTokens // Approve exactly the user's token amount
        );
          ...
          {isLoading && selectedProperty ? (
            ...
            <div className="property-details">
              ...
              <div className="actions-container">
                ...
                {/* Show Approval section ONLY if user is the property owner or if he has tokens */}
                {propertyDetails && parseFloat(propertyDetails.myBalance) > 0 && (
