...
function PropertyRent() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const detailsPromises = activeListings.map(async (listing) => {
          ...
          const isRented = await rentalContract.isPropertyRented(listing.propertyTokenAddress);
          const timeRemaining = isRented ? 
            await rentalContract.getRentalTimeRemaining(listing.propertyTokenAddress) : 0;
          
          return {
            address: listing.propertyTokenAddress,
            weeklyRate: ethers.formatEther(listing.weeklyRate), // Updated
            isActive: listing.isActive,
            isRented,
            timeRemaining: Number(timeRemaining), // Updated
            name
          };
        });
        ...
        for (const property of formattedProperties) {
          ...
          detailsMap[property.tokenAddress] = {
            symbol,
            balance: ethers.formatUnits(balance, decimals), // Updated
            totalSupply: ethers.formatUnits(totalSupply, decimals), // Updated
            decimals
          };
        }
  ...
  const handleListProperty = async (e) => {
    ...
    try {
      ...
      const weeklyRateWei = ethers.parseEther(weeklyRate); // Updated
              ...
              <div className="rentals-grid">
                {rentalListings.map((listing, index) => (
                  <div key={index} className="rental-card">
                    <h4>{listing.name}</h4>
                    <p><strong>Rate:</strong> {listing.weeklyRate} ETH/week (15 seconds for demo)</p>
                    <p><strong>Status:</strong> {listing.isRented ? 
                      `Rented (${formatTimeRemaining(listing.timeRemaining)})` : 
                      "Available"}
                    </p>
