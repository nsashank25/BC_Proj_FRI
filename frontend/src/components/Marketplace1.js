...
// --- End of REVISED Buy Tokens Function ---
  ...
  const handleApproveMarketplace = async () => {
    ...
    setApprovalStatus("Processing approval for maximum amount..."); // Update status message
    ...
    try {
        ...
        // Use ethers.MaxUint256 for ethers v6+ which provides the max value as a BigInt
        // For ethers v5, it would be ethers.constants.MaxUint256
        ...
        const approveTx = await tokenContract.approve(
            TOKEN_MARKETPLACE_ADDRESS,
            amountToApprove // Approve the maximum possible value
        );
        ...
        // Optionally check allowance afterwards for confirmation
        const decimals = await tokenContract.decimals(); // Needed for formatting check
        const newAllowance = await tokenContract.allowance(account, TOKEN_MARKETPLACE_ADDRESS);
        console.log(`New allowance check: ${ethers.formatUnits(newAllowance, decimals)} tokens (Should be very large or max)`);
